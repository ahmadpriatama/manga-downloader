...
foreach($animes as $keys => $anime) {
	...
	if ($useproxy) curl_setopt($curl, CURLOPT_PROXY, "creacle.ahmad-desktop:8888");
	...
	foreach($arr as $key => $item) {
		...
		if ($useproxy) curl_setopt($ch[$key], CURLOPT_PROXY, "creacle.ahmad-desktop:8888");
	...
	foreach ($ch as $key => $item) {
		...
		if ($useproxy) curl_setopt($ch2[$key], CURLOPT_PROXY, "creacle.ahmad-desktop:8888");
